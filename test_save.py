...
def test_save_only_4byte_utf8_over_64kb():
    """File with only 4-byte UTF-8 characters exceeding 64KB."""
    # Musical symbols are 4 bytes each
    content = "\U0001d11e" * (save.BUFFER_SIZE // 4 + 1)
    assert len(content.encode("utf-8")) > save.BUFFER_SIZE
...
def test_save_ascii_only_64kb():
    """64KB pure-ASCII file saves correctly (regression guard)."""
    content = "A" * save.BUFFER_SIZE
